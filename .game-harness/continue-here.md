...
- Latest live-provider slice: backend `openai-codex` proof now works through
  ...
  exposed by the response. Godot now also drives actual `PlayableSession`
  route-context packets for Store Clerk and Waiting Customer through live
  `/v1/npc/decision`, passes the Store Clerk live utterance into the Waiting
  Customer packet as observed context, records total usage, confirms no route
  mutation, and then proves clean-cover fallback parity. This is still
...
- Latest correction slice: active Store dialogue choices must remain speech
  ...
  Codex gameplay QA verifies `canReadEnvironmentToolCatalog=true`. This keeps
  LLM/NPC agency on tool selection plus runtime validation instead of
  per-choice result hardcoding.
- Latest implementation slice: Studio and Park rule boards are now readable
  ...
  31/31 accepted public actions, 33/33 explainability checks, and 5/5 route
  reports after the visible reaction-source update. This is a game-facing
...
- Latest implementation slice: the live HUD record line now also shows
...
- Latest implementation slice: Station Officer can now be inspected as the
  authority actor after inquest. Codex gameplay QA focuses the Officer,
  interacts, and reads `기록 인용`, `civic-ledger-6 -> civic-ledger-5`, the
  Station document target, `심문 초점`, `대상=플레이어`, comparison against the
