...
| Backend protects deterministic authority and rejects unavailable/hidden/forbidden provider actions. | `npm run check --prefix backend/npc-runtime` passed with 142 tests after the current Codex gameplay probe refresh. | PASS |
...
| Provider prompt treats NPC memory and policy as bounded context. | `npm run check --prefix backend/npc-runtime` passed with 142 tests; prompt coverage for `actorMemory` and `actorPolicy` still verifies the provider cannot infer hidden events, private intent, unobserved ledger facts, new affordances, authority, records, or state mutations. | PASS |
...
| Codex gameplay QA is current after conversation visible-context changes. | `$GODOT_BIN --headless --path godot --script res://tools/codex_gameplay_probe.gd` passed; status helper reports Codex source freshness pass; JSON SHA-256 `f683ae68d448ac326fe78045f0784bcec0eb80c1939add31387dfa5da5b878f5`. | PASS |
| Codex can play, inspect, and explain the proof cell through public APIs. | `data/evidence/godot/codex-gameplay-probe/dre_171_codex_gameplay_probe.json`: `ok=true`, `aiPlayerReport.pass=true`, `stage=inquest`, 31/31 accepted actions, 5/5 route reports, 29 explainability checks including conversation visible-context, Store Clerk record-action inspection, Store Manager handoff inspection, Park Witness public-spread inspection, Station Officer authority-citation inspection, Studio PM opportunity-change inspection, actor memory, and provider-packet actor policy readiness. | PASS |
