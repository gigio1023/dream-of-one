...
const checks = [
  ...
  ["latest ledger id", latestLedgerId === "civic-ledger-4", latestLedgerId],
  ["latest ledger citation", latestLedger.citedLedgerEventId === "civic-ledger-3", latestLedger.citedLedgerEventId],
  ...
  ["live HUD record chain", outcomeChecks.liveRecordChain === true && String(consequenceLabel).includes("플레이어 발화/응답 지연 -> 상점 기록 -> 보고 전달 -> 스테이션 인용"), consequenceLabel],
  ["outcome consequence chain", outcomeChecks.speechDelayRecordChain === true && String(outcomeBody).includes("플레이어 발화/응답 지연 -> 상점 기록 -> 보고 전달 -> 스테이션 인용 -> 심문"), outcomeBody],
