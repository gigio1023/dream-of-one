...
- [x] Make visible NPC reaction markers show which role action caused the
  reaction: the Waiting Customer world label now shows `접촉 거부` plus
  `← 스테이션 직원 · 기록 인용`, so the player can read the social source and
  action before opening HUD detail.
