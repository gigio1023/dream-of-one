...
- [x] Make visible NPC reaction markers show who caused the reaction: the
  Waiting Customer world label now shows `접촉 거부` plus `← 스테이션 직원`, so
  the player can read the social source before opening HUD detail.
