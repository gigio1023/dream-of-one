...
export interface GodotObservationRecordInput {
  sessionId: string;
  npcId?: string;
  eventName: string;
  kind: "world" | "action" | "domain";
  payload: Record<string, unknown>;
}
...
function hasPositionField(payload: Record<string, unknown>): boolean {
  const positionKeys = ["position", "targetPosition", "entityPosition", "newBlockPosition", "oldBlockPosition"] as const;
...
export class RuntimeTelemetryCollector {
  ...
  recordGodotObservation(input: GodotObservationRecordInput): void {
    this.pushRecord({
      type: "godot_observation",
  ...
  recordDecisionCycle(input: DecisionCycleRecordInput): void {
    this.pushRecord({
      ...
      payload: {
        ...
        actionType: input.dispatchResult?.actionType ?? null,
  ...
  buildEvidencePack(): RuntimeEvidencePack {
    ...
    for (const record of this.records) {
      ...
      if (record.type === "godot_observation") {
