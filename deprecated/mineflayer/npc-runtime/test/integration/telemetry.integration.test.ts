...
function buildDecision(overrides: Partial<DecisionEnvelope> = {}): DecisionEnvelope {
  return {
    intent: {
      ...
      confidence: 0.9,
      command: "chat",
      commandArgs: { message: "Queue procedure confirmed." },
    },
    meta: {
      usedFallback: false,
      reasonCategory: "none",
...
test("telemetry collector builds evidence summary for mineflayer and decisions", () => {
  const collector = new RuntimeTelemetryCollector({ maxRecords: 1000 });
  collector.recordMineflayerEvent({
    kind: "world",
    eventName: "blockUpdate",
    occurredAt: new Date().toISOString(),
    payload: {
      newBlockPosition: { x: 1, y: 64, z: 1 },
    },
  });
  collector.recordMineflayerEvent({
    kind: "action",
    eventName: "diggingCompleted",
    occurredAt: new Date().toISOString(),
    payload: {
      entityPosition: { x: 2, y: 64, z: 2 },
    },
  });
  collector.recordDecisionCycle({
    requestId: "req-1",
    ...
    dispatchResult: {
      actionId: "req-1",
      command: { type: "chat", args: { message: "Queue procedure confirmed." } },
      result: {
        ok: true,
        actionId: "req-1",
        evidence: { eventName: "chat" },
      },
    },
  });
  ...
  assert.equal(evidence.countsByType.mineflayer_event, 2);
  assert.equal(evidence.countsByType.decision_cycle, 1);
  assert.equal(evidence.worldObservationSummary.worldEvents, 1);
  assert.equal(evidence.worldObservationSummary.actionEvents, 1);
  assert.equal(evidence.worldObservationSummary.nearbyNpcObservations, 2);
...
test("telemetry collector exports evidence pack JSON file", async () => {
  ...
  collector.recordDecisionCycle({
    ...
    decision: buildDecision({
      meta: {
        usedFallback: true,
        reason: "policy_command_not_allowed",
