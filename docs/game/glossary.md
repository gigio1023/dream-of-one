...
| **Storylet** | A small self-contained social scenario (prompt set, choices, signals, routes). Canon lives in `docs/scenario/`. `Same Order` (같은 주문) is the first storylet. |
...
| **Fallback** | Deterministic text/behavior used when providers are off, unavailable, over budget, or a proposal fails validation. Always present. |
