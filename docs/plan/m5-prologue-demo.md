...
`fr-FR`, and `ja-JP` across authored/generated text and provider-interruption UI. The M3R
...
testing); first-run provider setup UX that clearly requires a working profile
or stops at the visible interruption surface; crash-safe session handling;
...
control, and how provider interruption behaves), limitations list, credits
...
- [ ] Zero-key first run clearly stops without inventing any social event and
      guides the player to provider setup or a clean restart.
...
- [ ] All six target locales have complete authored content and one full-run
      parity pass; generated text and provider-interruption UI use the selected
      run locale.
