...
- Once implementation and self-review are complete, Sol writes one bounded
  ...
  The lead session stays in place and native spawn selects the tracked
  `terra_high_playtest` custom agent role, whose config pins GPT-5.6 Terra at
  high effort, for the actual game drive through Godot AI and the fun/acceptance
  observations. Terra does not edit or diagnose on failure; it reports, Sol
  repairs, and a new bounded Terra run is issued. If the trusted-project session
  does not expose that role, live acceptance is blocked; changing the parent or
  naming Terra only in prompt prose is not a substitute.
