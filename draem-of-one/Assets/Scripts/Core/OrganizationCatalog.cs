...
namespace DreamOfOne.Core
{
    ...
    {
        private static readonly Dictionary<OrganizationId, OrganizationDefinition> definitions = new();
        ...
        static OrganizationCatalog()
        {
            ...
            Add(new OrganizationDefinition
            {
                ...
                roles = new[] { RoleId.FacilityTech }
            });

            Add(new OrganizationDefinition
            {
                id = OrganizationId.Media,
                goal = "촬영 허가 준수/촬영 구역 안전 유지",
                procedures = new[] { "사전 허가", "구역 표시", "촬영", "반납" },
                resources = new[] { "촬영 장비", "허가서" },
                artifacts = new[] { "촬영 허가서", "촬영 로그" },
                roles = new[] { RoleId.Reporter }
            });
        }

        public static bool TryGet(OrganizationId id, out OrganizationDefinition definition)
        {
            if (id != OrganizationId.None && definitions.TryGetValue(id, out definition))
            {
                return true;
            }
        ...
        {
            if (definition.id == OrganizationId.None)
            {
                return;
            }
