...
const BOTTOM_CONVERSATION_TOP := -360.0
...
const UI_COPY := {
	"ko": {
		...
		"record_chain_repair": "흐름: 기억 공백 발화 -> 정정표 -> 대기줄 수습 -> 공개 수습 게시 -> 조건부 리뷰",
	...
	"en": {
		...
		"record_chain_repair": "Chain: memory-gap speech -> correction slip -> queue repaired -> public repair notice -> conditional review",
