...
const OUTPUT_PATH := "res://../data/evidence/godot/live-provider-dispatch/dre_171_live_provider_thread_continuity_smoke.json"
const ARTIFACT_PATH := "data/evidence/godot/live-provider-dispatch/dre_171_live_provider_thread_continuity_smoke.json"
const OUTPUT_ENV := "DREAM_OF_ONE_LIVE_PROVIDER_THREAD_CONTINUITY_OUTPUT"
const MAX_PROVIDER_ESTIMATED_COST_USD := 0.01
const MAX_PROVIDER_TOTAL_ESTIMATED_COST_USD := 0.01
...
func _run() -> void:
	...
	_require(readiness["ok"], failures, "Live backend readiness must pass before spending provider decision requests.")
	...
	var session_id := "godot-live-thread-continuity-%d-%d" % [
	...
	if failures.is_empty():
		...
		first_packet = session.call("debug_live_provider_packet", session_id, "NPC_Store_Clerk")
		first_report = await BackendBridge.probe_live_decision(
			self,
			first_packet,
			decision_endpoint,
			BackendBridge.DEFAULT_DECISION_TIMEOUT_MS
		)
		_validate_decision_report(first_report, first_packet, "codex", failures)
	if failures.is_empty():
		second_packet = session.call("debug_live_provider_packet", session_id, "NPC_Store_Clerk")
		second_report = await BackendBridge.probe_live_decision(
			self,
			second_packet,
			decision_endpoint,
			BackendBridge.DEFAULT_DECISION_TIMEOUT_MS
		)
		_validate_decision_report(second_report, second_packet, "codex-reply", failures)
		route_after = _session_summary(session)
		route_after_fingerprint = _route_fingerprint(route_after)
		_require(route_before_fingerprint == route_after_fingerprint, failures, "Live provider thread-continuity probe mutated PlayableSession route state.")
		_validate_product_provider_state(route_after, failures)

	var usage_totals := _provider_usage_totals([
		first_report.get("decision", {}).get("meta", {}).get("providerUsage", {}),
		second_report.get("decision", {}).get("meta", {}).get("providerUsage", {})
	])
	if not first_report.is_empty() and not second_report.is_empty():
		_require(float(usage_totals.get("estimatedCostUsd", 1.0)) <= MAX_PROVIDER_TOTAL_ESTIMATED_COST_USD, failures, "Two-call live provider continuity proof must stay under the total estimated cap.")

	var first_summary := _summarize_decision(first_report)
	var second_summary := _summarize_decision(second_report)
	var after_fingerprint := JSON.stringify(context)
	_require(before_fingerprint == after_fingerprint, failures, "Live provider thread-continuity smoke mutated runtime context.")

	var pack := {
		"ok": failures.is_empty(),
		"runId": "dre-live-provider-thread-continuity-smoke",
		"schemaVersion": ShellSchema.EVIDENCE_SCHEMA,
		"adapter": "godot",
		"checks": {
			"readiness": _summarize_readiness(readiness),
			"firstDecision": first_summary,
			"secondDecision": second_summary,
			"playableRoute": {
				"before": _summarize_route(route_before),
				"after": _summarize_route(route_after),
				"stateMutatedByProviderDecision": route_before_fingerprint != route_after_fingerprint
			}
		},
		"proofs": {
			"decisionRequested": not first_report.is_empty() and not second_report.is_empty(),
			"decisionCount": int(not first_report.is_empty()) + int(not second_report.is_empty()),
			"sessionId": session_id,
			"npcId": "NPC_Store_Clerk",
			"sameSessionNpc": str(first_packet.get("sessionId", "")) == session_id and str(second_packet.get("sessionId", "")) == session_id and str(first_packet.get("npcId", "")) == "NPC_Store_Clerk" and str(second_packet.get("npcId", "")) == "NPC_Store_Clerk",
			"firstTransport": str(first_summary.get("transport", "")),
			"secondTransport": str(second_summary.get("transport", "")),
			"firstThreadId": str(first_summary.get("threadId", "")),
			"secondThreadId": str(second_summary.get("threadId", "")),
			"threadContinuity": str(first_summary.get("transport", "")) == "codex" and str(second_summary.get("transport", "")) == "codex-reply" and not str(first_summary.get("threadId", "")).is_empty() and not str(second_summary.get("threadId", "")).is_empty(),
			"providerMode": "openai-codex",
			"selectedModel": str(first_summary.get("providerUsage", {}).get("model", "")),
			"usedFallback": bool(first_summary.get("usedFallback", true)) or bool(second_summary.get("usedFallback", true)),
	...
	if file == null:
		failures.append("Unable to write live provider thread-continuity evidence: %s" % error_string(FileAccess.get_open_error()))
		pack["ok"] = false
...
func _mock_context() -> Dictionary:
	return {
		"sessionId": "dre-live-provider-thread-continuity-smoke-session",
...
func _validate_decision_report(report: Dictionary, provider_packet: Dictionary, expected_transport: String, failures: Array[String]) -> void:
	...
	_require(str(meta.get("transport", "")) == expected_transport, failures, "Live provider decision must use %s transport." % expected_transport)
	_require(not str(meta.get("threadId", "")).is_empty(), failures, "Live provider decision must report a thread id.")
	_require(str(provider_usage.get("model", "")) == "gpt-5.4-mini", failures, "Live provider decision must use gpt-5.4-mini.")
	_require(float(provider_usage.get("estimatedCostUsd", 1.0)) <= MAX_PROVIDER_ESTIMATED_COST_USD, failures, "Live provider decision must stay under the per-request estimated cap.")
...
func _validate_product_provider_state(summary: Dictionary, failures: Array[String]) -> void:
