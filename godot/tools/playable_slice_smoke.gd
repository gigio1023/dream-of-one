...
const ROUTE_DEFINITIONS := [
	...
	},
	{
		...
		"expectedStationIntake": false,
		"expectedStationInquest": false,
		"expectedRecordStates": {"store_queue_mark": "distanced", "receipt_tray": "marked", "correction_slip": "absent", "report_tray": "empty", "park_notice_board": "warned", "studio_review_queue": "deferred", "station_dossier": "absent"},
...
func _validate_route_summary(route: Dictionary, summary: Dictionary) -> Array[String]:
	...
	if route_id == "cover_held_under_suspicion":
		...
		if not str(summary.get("outcomeBody", "")).contains("스튜디오 리뷰 보류"):
			failures.append("cover_held_under_suspicion outcome must show the public warning deferring the Studio review")
	return failures
...
func _validate_agent_action_log(route: Dictionary, summary: Dictionary) -> Array[String]:
	...
	if route_id == "cover_held_under_suspicion" and not _agent_action_exists(action_log, "studio_pm", "defer_review"):
		failures.append("cover_held_under_suspicion expected Studio PM defer_review action after public warning")
	if route_id == "cover_held_under_suspicion" and not _agent_action_perceives(action_log, "studio_pm", "defer_review", "park_notice_board"):
		failures.append("cover_held_under_suspicion expected Studio PM to perceive park_notice_board before deferring review")
	if route_id == "cover_held_under_suspicion" and not _agent_action_perceives(action_log, "studio_pm", "defer_review", "studio_review_queue"):
		failures.append("cover_held_under_suspicion expected Studio PM to perceive studio_review_queue before deferring review")
	...
	if route_id == "cover_held_under_suspicion" and not _social_observation_exists(social_observations, "studio_pm", "park_witness", "post_warning", "defer_review"):
		failures.append("cover_held_under_suspicion expected Studio PM reading the public warning before deferring review")
...
func _validate_visible_social_actors(route: Dictionary, summary: Dictionary, session: Node) -> Array[String]:
	...
	if route_id == "cover_held_under_suspicion" and _agent_action_actor_exists(action_log, "NPC_Studio_PM"):
		var studio_state: Dictionary = visible_states.get("NPC_Studio_PM", {})
		if str(studio_state.get("state", "")) != "deferred":
			failures.append("cover_held_under_suspicion expected NPC_Studio_PM visible state to show deferred review")
		if not bool(studio_state.get("markerVisible", false)):
			failures.append("cover_held_under_suspicion expected NPC_Studio_PM reaction marker after review deferral")
		if not str(studio_state.get("reactionText", "")).contains("리뷰"):
			failures.append("cover_held_under_suspicion expected NPC_Studio_PM reaction label to mention review deferral")
	return failures
...
func _validate_hud_record_state(route: Dictionary, summary: Dictionary, hud: Node) -> Array[String]:
	...
	if ["clean_cover", "repair_recovered", "cover_held_under_suspicion", "soft_report", "inquest_opened"].has(route_id) and not record_state_label.contains("열람"):
		failures.append("%s expected HUD record state to show which roles can read current records" % route_id)
	...
	if route_id == "inquest_opened" and (not record_state_label.contains("스테이션 직원") or not record_state_label.contains("스튜디오 PM") or not record_state_label.contains("대기 손님")):
		failures.append("inquest_opened expected HUD record readers to name Station Officer, Studio PM, and Waiting Customer")
...
func _agentic_route_proof(route_id: String) -> Dictionary:
	if route_id == "clean_cover":
		...
		return _agentic_route_result(route_id, "cover_held", "clean_cover_line", "네, 같은 걸로 주세요.", "The clerk closes a normal receipt, a waiting customer accepts the routine, a Park witness publicly vouches that the player stayed in the local flow, high local trust lets the waiting customer share a local tip, and the Studio PM opens a tiny review invite from that public vouch.", clean_trace, _agentic_final_states({"store_queue_mark": "helped", "usual_order_cue": "cited", "receipt_tray": "normal", "park_notice_board": "vouched", "studio_review_queue": "invited"}))
	if route_id == "repair_recovered":
		var repair_trace := [
	...
	if route_id == "inquest_opened":
		var inquest_trace := [
			...
			_agentic_trace("inquest.waiting_customer.refuse_contact", "NPC_Waiting_Customer", "waiting_customer", ["store_queue_mark", "store_counter", "usual_order_cue", "park_notice_board"], "refuse_contact", "store_queue_mark", "civic-ledger-8", "queue_contact_refused", {"recordId": "store_same_order_contact_refused", "citedLedgerEventId": "civic-ledger-6"}, _economy(3, 0, 93, 70), "A waiting customer sees the Station cite the player and refuses contact while the inquest is open.")
		]
