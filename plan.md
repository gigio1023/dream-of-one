...
### 2.5 영감 요소 반영 (가안)
- 규칙 예시: “꿈” 단어 금지(Language)
  - `id`: `RUL-LAN-NoDreamWord`
  - `statement`: “대화에서 ‘꿈’ 단어를 사용하면 안 된다.”
  - `violation`: `say:dream`
  - `clues`: 포스터(‘현실 존중’), NPC 완곡 화법(“그 단어는 삼가요”), 소곤거림
  - `suspicion_delta`: 15 (초심자 기준 체감치)
- 연동: 해당 위반이 목격되면 NPC 개인 의심이 급상승하고, **디렉터**는 “집중 시선” 연출 이벤트를 우선 트리거(정보/긴장 분배)
...
- **규칙 프리셋 12개**(세션에선 랜덤 3개 사용)
    - 예) “왼손 인사”, “문은 2회 밀기”, “대화 시작은 호칭만”, “벤치는 오른쪽부터 채운다”, “정각 10초 침묵”, “날씨 화제 금지”, “'꿈' 단어 금지(가안)” …
